(function($) {
    $.fn.html5Uploader = function(options) {
            ...
            if ($this.is("[type='file']")) {
                ...
                function() {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        if(settings.setParams) {
                            settings.setParams(files[i]);
                        }
        ...
        function fileHandler(file) {
            ...
            if (file.getAsBinary) {
                var data = "";
                for (key in settings.postParams) {
                    data += dashes + boundary + crlf + "Content-Disposition: form-data;" + "name=\"" + key + "\"";
                    data += crlf + crlf + unescape(encodeURIComponent(settings.postParams[key])) + crlf;
                }

                data += (dashes + boundary + crlf + "Content-Disposition: form-data;" + "name=\"" + settings.name + "\";" + "filename=\"" + unescape(encodeURIComponent(file.name)) + "\"" + crlf + "Content-Type: application/octet-stream" + crlf + crlf + file.getAsBinary() + crlf + dashes + boundary + dashes);
