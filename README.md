...
└── src                             // 项目源码目录
    ...
    │   └── index.go                // 1. 习惯性的将文件名与要处理的url路径名相同(通过App.RegisterController注册路径的Handle)，来清晰的划分功能模块， 注册响应要处理的url目录路径，然后处理子页面的逻辑。 2. 文件名和App.RegisterController注册的路径的Handle可以不同，主要依据App.RegisterController注册的路径的Handle。 3. 每个模块用一个文件名，用来功能性分离， 让结构更清晰而已。
    ├── data                        // 存放App自有数据的目录
    ├── doc.go                      // Go的文档文件
